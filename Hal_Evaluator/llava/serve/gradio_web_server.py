...
headers = {"User-Agent": "Shell-V Client"}
...
def http_bot(state, model_selector, temperature, top_p, max_new_tokens, request: gr.Request):
    ...
    if len(state.messages) == state.offset + 2:
        ...
        if "llava" in model_name.lower():
            if 'llama-2' in model_name.lower():
                ...
                template_name = "mpt"
            elif 'codeshell' in model_name.lower():
                template_name = "codeshell"
            else:
                if 'mmtag' in model_name.lower():
    ...
    pload = {
        ...
        "stop": state.sep if state.sep_style in [SeparatorStyle.SINGLE, SeparatorStyle.MPT, SeparatorStyle.CODESHELL] else state.sep2,
...
title_markdown = ("""
#  Shell-v: Large Language and Vision Assistant
""")
...
def build_demo(embed_mode):
    ...
    with gr.Blocks(title="Shell-v", theme=gr.themes.Default(), css=block_css) as demo:
        state = gr.State()
        ...
        with gr.Row():
            with gr.Column(scale=3):
                ...
                gr.Examples(examples=[
                    [f"{cur_dir}/examples/dog.png", "What kind of scene is depicted in this picture?"],
                    [f"{cur_dir}/examples/COCO_train2014_000000435579.jpg", "What sport is shown in the picture?"],
            ...
            with gr.Column(scale=8):
                chatbot = gr.Chatbot(elem_id="chatbot", label="Shell-v Chatbot", height=550)
                with gr.Row():
                    with gr.Column(scale=8):
                        textbox.render()
