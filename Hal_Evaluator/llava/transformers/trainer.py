...
class Trainer:
    ...
    def _inner_training_loop(
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                with self.accelerator.accumulate(model):
                    tr_loss_step , tr_loss_step_dict= self.training_step(model, inputs)
                ...
                if (
                    ...
                    for _key in tr_loss_step_dict.keys():
                        if _key not in tr_loss_dict.keys():
                            tr_loss_dict[_key] = tr_loss_step_dict[_key] / (
                ...
                if (
                    ...
                    self._maybe_log_save_evaluate(tr_loss,tr_loss_dict, model, trial, epoch, ignore_keys_for_eval)
                else:
        ...
        for _key in tr_loss_dict.keys():
            self._total_loss_dict_scalar[_key] += tr_loss_dict[_key].item()
    ...
    def _maybe_log_save_evaluate(self, tr_loss, tr_loss_dict ,model, trial, epoch, ignore_keys_for_eval):
        if self.control.should_log:
            ...
            tr_loss_dict_scalar = {}
            for _key in tr_loss_dict.keys():
                tr_loss_dict_scalar[_key] = self._nested_gather(tr_loss_dict[_key]).mean().item()
            # reset tr_loss to zero
            tr_loss -= tr_loss

            for _key in tr_loss_dict.keys():
                tr_loss_dict[_key] -= tr_loss_dict[_key]
            

            logs["loss"] = round(tr_loss_scalar / (self.state.global_step - self._globalstep_last_logged), 4)
            logs["learning_rate"] = self._get_learning_rate()
            
            for _key in tr_loss_dict_scalar.keys():
                logs[_key] = round(tr_loss_dict_scalar[_key] / (self.state.global_step - self._globalstep_last_logged), 4)
 
            self._total_loss_scalar += tr_loss_scalar
            for _key in tr_loss_dict.keys():
                if _key not in self._total_loss_dict_scalar.keys():
                    self._total_loss_dict_scalar[_key] = tr_loss_dict_scalar[_key]
                else:
                    self._total_loss_dict_scalar[_key] += tr_loss_dict_scalar[_key]
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]]) -> torch.Tensor:
        ...
        loss_dict = outputs['loss_dict'] if isinstance(outputs, dict) else outputs[-1]
        for _key in loss_dict.keys():
            loss_dict[_key] = loss_dict[_key].mean()/self.args.gradient_accumulation_steps
